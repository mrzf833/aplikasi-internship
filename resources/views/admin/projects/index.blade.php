...
@section('btn-add')
<div>
    <button type="button" class="au-btn au-btn-icon au-btn--blue" data-toggle="modal" data-target="#add-project">
            ...
            <form action="{{ route('admin.projects.store') }}" method="post">
                ...
                <div class="modal-body">
                    <div class=" form-group has-success">
                        <label for="fullname" class=" control-label mb-1">Student</label>
                        <input type="text" name="fullname" id="fullname" class="form-control" autocomplete="off">
                    </div>
                    <div class=" form-group has-success">
                        <label for="email" class=" control-label mb-1">Mentor</label>
...
<div class="modal fade" id="edit-instructor-{{ $project->id }}" tabindex="-1" role="dialog" aria-labelledby="scrollmodalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Edit Instructor {{ $project->fullname }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ route('admin.instructors.edit',$project->id) }}" method="post">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class=" form-group has-success">
                        <label for="fullname" class=" control-label mb-1">Nama Lengkap</label>
                        <input type="text" name="fullname" id="fullname" class="form-control" autocomplete="off" content="{{ $project->fullname }}">
                    </div>
                    <div class=" form-group has-success">
                        <label for="email" class=" control-label mb-1">Email</label>
                        <input type="email" name="email" id="email" class="form-control" content="{{ $project->email }}">
                    </div>
                    <div class=" form-group has-success">
                            ...
                            <tbody>
                                @forelse ($projects as $project)
                                    <tr>
                                        ...
                                        <td>{{ $project->email }}</td>
                                        <td>
                                            <button type="button" class="btn btn-warning edit" data-toggle="modal" data-target="#edit-instructor-{{ $project->id }}">Edit</button>
